...
class WorkflowProcess(CompanyLookupMixin, models.Model):
    ...
    def _get_or_message(self, rules, new_state):
        message_list = [self._get_message_for_condition(rule, new_state) for rule in rules
                               if self._get_message_for_condition(rule, new_state)]
        
        return [item for sublist in message_list for item in 
    ...
    def _get_function_name(self, func):
        # Attempt to get the short_description from the function or method
        short_description = getattr(getattr(self, func), 'short_description', None)

        if short_description is not None:
            return str(short_description)

        # If not found, try to get it from the instance's attributes
        short_description = getattr(self, f"{func}_short_description", None)

        if short_description is not None:
            return short_description

        # If not found in either place, return the string representation of the function
        return str(func)
