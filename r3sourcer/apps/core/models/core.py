...
class Contact(CategoryFolderMixin,
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.languages.filter(default=True).first():
            if country_code:
                language_id = country_code.lower()
                if language_id not in ['et', 'fi', 'ru']:
                    language_id = 'en'
            else:
...
class UserManager(AbstractObjectOwnerManager, BaseUserManager):
    ...
    def _create_user(self, password, **extra_fields):
        ...
        country_code = extra_fields.pop('country_code', None)
        ...
        obj.contact.save(country_code=country_code)
        return obj
...
class Company(CategoryFolderMixin,
    ...
    def is_price_list_rules_set(self):
        return self.price_lists.filter(price_list_rates__rate__isnull=False).exists()
    
    def is_price_list_active(self):
        return self.price_lists.filter(effective=True).exists()    
    
    def is_price_list_exsist(self):
        return self.price_lists.all().exists()   
        
    def is_price_list_set(self):
        return self.is_price_list_exsist() and self.is_price_list_active() and self.is_price_list_rules_set()
    ...
    def save(self, *args, **kwargs):
        ...
        if just_added and self.type == self.COMPANY_TYPES.master:
            ...
            # if self.primary_contact.contact.languages.exists():
            if self.primary_contact:
                primary_contact = self.primary_contact
                languages_manager = primary_contact.contact.languages
                first_language = languages_manager.order_by('-default').first()
                language_id = first_language.language_id if first_language else settings.DEFAULT_LANGUAGE
            else:
...
class CompanyRel(UUIDModel,
    ...
    @workflow_function
    def is_price_list_set(self, short_description=None):
        messages = ['At least one price list is required.',
                    'At least one active price list is required.',
                    'At least one skill activity rate is required in the active price list.']
        checks = [
           self.regular_company.is_price_list_exsist(),
            self.regular_company.is_price_list_active(),
            self.regular_company.is_price_list_rules_set()
        ]

        is_price_list = all(result for result in checks)
        description_list = [description for description, result in zip(messages, checks) if result==False]

        setattr(self, f"{self.is_price_list_set.__name__}_short_description", description_list)

        return is_price_list
